...
use actix_web::{
    Error, HttpRequest, HttpResponse, dev::PeerAddr, error, web,
};
use awc::Client;
use url::Url;
...
fn build_x_forwarded_for(
    ...
    if trusted.is_trusted(&addr.ip()) {
        match req
            ...
            .and_then(|v| v.to_str().ok())
        {
            Some(existing) if !existing.is_empty() => Some(format!("{existing}, {this_hop}")),
            _ => Some(this_hop),
        }
    } else {
        Some(this_hop)
...
pub async fn forward(
    ...
    let forwarded_req = match build_x_forwarded_for(&req, peer_addr.as_ref(), &trusted_proxies) {
